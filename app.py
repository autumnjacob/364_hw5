...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/jacobauHW5"
## Provided:
...
def update(item):
    ...
    if form.validate_on_submit():
        new_items = TodoItem.query.filter_by(description=item).first()
        new_items.priority = form.priority.data
        db.session.commit()
...
def delete(lst):
    list = TodoList.query.filter_by(id=lst)
    name = list.first().title
    db.session.delete(list.first())
    db.session.commit()
